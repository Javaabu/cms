...
abstract class AllowedMimeTypes
{
    ...
    {
        // try using mime type
        $icon = self::$icons[$mime_type] ?? null;

        // try using type
        if (! $icon) {
            $icon = self::$icons[self::getType($mime_type)] ?? null;
        }
     ...
     * @return string|null
     */
    public static function getType($mime_type)
    {
        foreach (self::$allowed_mime_types as $type => $mime_types) {
    ...
    /**
     * Get the web icon based on mime type
     *
     * @param string $mime_type
     * @return string|null
     */
    public static function getWebIcon($mime_type)
    {
        // try using mime type
        $icon = self::$web_icons[$mime_type] ?? null;

        // try using type
        if (! $icon) {
            $icon = self::$web_icons[self::getType($mime_type)] ?? null;
        }
     ...
     * @return boolean
     */
    public static function isAllowedMimeType($mime_type, $type = '')
    {
     ...
     * @return array
     */
    public static function getAllowedMimeTypes($type = '')
    {
        return $type ? self::$allowed_mime_types[$type] ?? [] :
            Arr::flatten(self::$allowed_mime_types);
    }
     ...
     * @return string|array
     */
    public static function getValidationRule($type, $as_array = true)
    {
     ...
     * @return string
     */
    public static function getAllowedMimeTypesString($type = '', $separator = ',')
    {
