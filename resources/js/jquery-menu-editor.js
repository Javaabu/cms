...
function MenuEditor(idSelector, options) {
    ...
    function editItem($item) {
        ...
        var model_title = data.model_title;

        if (model_id && model_title) {
            var model_title = $('<div />').text(model_title).html();

            $form.find("[name=model_id]")
                .html('<option value="' + model_id + '">' + model_title + '</option>')
                .val(model_id)
                .trigger('change');
        }
    ...
    this.update = function () {
        ...
        $form.find('.item-menu').each(function () {
            var curr_val = $(this).attr('type') == 'checkbox' ? $(this).prop('checked') : $(this).val();

            if ($(this).prop('disabled')) {
                curr_val = null;
            }

            $cEl.data($(this).attr('name'), curr_val);
        });
